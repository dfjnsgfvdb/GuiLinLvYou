...
async def query_user_qa_record(request):
    ...
    detail = bool(request.json.get("detail"))
    user_info = await get_user_info(request)
    return await query_user_record(user_info["id"], page, limit, search_text, chat_id, detail)
...
async def delete_user_qa_record(request):
    ...
    :return:
    """
    chat_ids = request.json.get("chat_ids") or request.json.get("record_ids")
    user_info = await get_user_info(request)
    return await delete_user_record(user_info["id"], chat_ids)
